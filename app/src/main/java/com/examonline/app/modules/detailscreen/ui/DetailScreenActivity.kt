...
public class DetailScreenActivity :
  ...
  public override fun onInitialized(): Unit {
    ...
    when {
        intent.getStringExtra("DoingFlag") == "Done" -> {
        ...
        intent.getBooleanExtra("Expired",false) -> {
          binding.btnStartExam.text = MyApp.getInstance().getString(R.string.lbl_expired)
          binding.btnStartExam.setTextColor(Color.RED)
          binding.btnStartExam.isClickable = false
        }
        intent.getStringExtra("Begin") == "true" -> {
          binding.btnStartExam.text = MyApp.getInstance().getString(R.string.lbl_upcoming)
          binding.btnStartExam.setOnClickListener { alert(
            MyApp.getInstance().getString(R.string.lbl_notime),
            MyApp.getInstance().getString(R.string.msg_notime)
          ) {
            neutralButton {
            }
          }
